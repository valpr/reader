...
>
  <div
    ...
    class="grid grid-cols-1 gap-2 w-full max-w-full py-1"
  >
    <div
      role="radio"
      ...
      class="relative flex items-start gap-2.5 p-2.5 rounded-lg border text-left transition-all duration-150 cursor-pointer select-none min-w-0 w-full max-w-full overflow-hidden break-words [overflow-wrap:anywhere]"
    ...
    >
      ...
      <span class="flex flex-col min-w-0 flex-1">
        ...
        <span class="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
          Compact layout with tap-edge page turning (17px font)
        </span>
      </span>
    ...
    <div
      role="radio"
      aria-checked={selectedId === 'default-tablet'}
      tabindex="0"
      data-testid="profile-choice-tablet"
      on:click={() => select('default-tablet')}
      on:keydown={(e) => handleKey(e, 'default-tablet')}
      class="relative flex items-start gap-2.5 p-2.5 rounded-lg border text-left transition-all duration-150 cursor-pointer select-none min-w-0 w-full max-w-full overflow-hidden break-words [overflow-wrap:anywhere]"
      class:border-zinc-800={selectedId === 'default-tablet'}
      class:dark:border-zinc-200={selectedId === 'default-tablet'}
      class:shadow-sm={selectedId === 'default-tablet'}
      class:border-zinc-200={selectedId !== 'default-tablet'}
      class:dark:border-zinc-700={selectedId !== 'default-tablet'}
      style="background-color: {selectedId === 'default-tablet'
        ? 'var(--astryx-color-surface-selected, rgba(255, 255, 255, 0.08))'
        : 'var(--astryx-color-surface, transparent)'};"
    >
      <span
      ...
      <span class="flex flex-col min-w-0 flex-1">
        ...
        <span class="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
          Spacious touch layout with generous margins (22px font)
        </span>
      </span>
      {#if selectedId === 'default-tablet'}
        <span
          class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[11px] font-semibold bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800 shrink-0"
        >
          <Fa icon={faCheck} class="text-[9px]" />
    ...
    >
      ...
      <span class="flex flex-col min-w-0 flex-1">
        ...
        <span class="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
          High contrast for E-Ink devices (20px font, 500 weight)
        </span>
      </span>
