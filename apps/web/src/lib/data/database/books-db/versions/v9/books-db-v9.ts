...
export interface BooksDbV9StatisticData {
  ...
  /** Characters read during each hour of the day (0..23). Array length 24. */
  charactersByHour?: number[];
  /** Total dictionary/vocabulary lookups triggered during reading on this date. */
  lookupCount?: number;
  /** Dictionary lookups triggered during each hour of the day (0..23). Array length 24. */
  lookupsByHour?: number[];
  ...
  maxProgress?: number;
  /** Reading time in seconds bucketed by reader profile ID. */
  readingTimeByProfile?: Record<string, number>;
}
